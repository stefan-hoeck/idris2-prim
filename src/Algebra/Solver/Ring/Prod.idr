...
public export
eprod : RingLaws a => {as : List a} -> Prod a as -> a
eprod {as = []}      []        = 1
...
export
0 pone : RingLaws a => (as : List a) -> eprod (one {as}) === 1
pone []        = Refl
pone (x :: xs) = Calc $
  |~ 1 * eprod (one {as = xs})
  ~~ 1 * 1                     ...(cong (1 * ) $ pone xs)
  ~~ 1                         ...(multOneRightNeutral _)
...
pvar (x :: vs) Here      = Calc $
  ...
  ~~ x * 1                           ...(multOneRightNeutral _)
  ~~ x                               ...(multOneRightNeutral _)

pvar (v :: vs) (There y) = Calc $
  |~ 1 * eprod (fromVar {as = vs} y)
  ~~ 1 * x                           ...(cong (1*) $ pvar vs y)
  ~~ x                               ...(multOneLeftNeutral _)
...
pmerge []        []        = sym $ multOneLeftNeutral _
